...
def load_and_process_data(file):
    ...
    try:
        ...
        # Extract month from POD DATE/TIME instead of ORD CREATE
        if 'POD DATE/TIME' in df.columns:
            df['Month'] = df['POD DATE/TIME'].dt.to_period('M').astype(str)
        elif 'ORD CREATE' in df.columns:
            # Fallback to ORD CREATE if POD not available
